...
fn main() {
    ...
    let matched_stars = [51u32, 376, 306, 272, 321, 1075, 1427, 1231, 3550, 2851, 3784, 2444, 1215, 5733, 1886];
    ...
    for i in 0..star_positions.len() {
        for j in (i+1)..star_positions.len() {
            let sep = angular_separation(&star_positions[i].1, &star_positions[j].1)
                .to_degrees();
            if sep < 30.0 {
                println!("  Stars {} - {}: {:.2}°", star_positions[i].0, star_positions[j].0, sep);
            }
        }
    }
    println!();
    ...
    println!("Angular ratios (sorted): [{:.4}, {:.4}, {:.4}, {:.4}, {:.4}]",
        ratios[0], ratios[1], ratios[2], ratios[3], ratios[4]);
    println!();

    // Check if this is within the FOV range
    println!("Is max distance ({:.2}°) within FOV range ({:.1}° - {:.1}°)? {}",
        max_dist, fov_min, fov_max,
        if max_dist >= fov_min as f64 && max_dist <= fov_max as f64 { "YES" } else { "NO" });
    println!();
    ...
    println!("\nFound {} patterns with 3+ matched stars", patterns_with_matched);
    println!("Found {} patterns with all 4 stars in matched set", patterns_with_4_matched);
    ...
    println!("  Angular ratios (computed):  [{:.4}, {:.4}, {:.4}, {:.4}, {:.4}]",
        ratios[0], ratios[1], ratios[2], ratios[3], ratios[4]);
}
